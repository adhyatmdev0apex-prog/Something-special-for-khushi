...
const UNIVERSE_SEQ = [
    ...
    { text:"Searching Memory Traces...",       speed:30, pause:700,  stars:8  },
    { text:"Cross-Referencing Memories...",    speed:30, pause:900,  stars:12 },
    { text:"Target Identified",                speed:18, pause:1500, stars:25 },
    { text:"Preparing Something For You...", speed:25, pause:900,  stars:15 },
...
/*=========================================================
    PAUSE  —  quiet transition before the confession
=========================================================*/
function showCake(){
    loadTemplate("pause-template");
    _runPause();
}

async function _runPause(){
    changeSky("night");

    const line = document.querySelector(".pauseScene .pauseLine");
    if(line){
        line.style.opacity = "0";
        await wait(400);
        line.style.transition = "opacity 1.6s ease";
...
const MEMORY_PAGES = [
    ...
    { title:"A Door Opens", kicker:"Chapter 03", body:"Out of 8 billion people, today found one name.", icon:"✨" },
    { title:"Today's Mission", kicker:"Chapter 04", body:"Smile. Laugh a lot. Enjoy every moment.", icon:"✓" },
    ...
    { title:"A Little Reveal", kicker:"Chapter 07", body:"Candles, music, and a tiny universe quietly showing what it felt.", icon:"♫" },
...
async function _runMemory(){
    ...
    app.innerHTML = `
      <section class="scene memoryScene">
        ...
        <div id="constellationTitle" class="constellationTitle">FOR KHUSHI</div>
...
/*=========================================================
    CONFESSION SCENE  —  final date question
=========================================================*/
function showConfession(){
    loadTemplate("confession-template");
    _runConfession();
}

function _runConfession(){
    changeSky("night");

    const yesBtn  = document.getElementById("confessYes");
    const noBtn   = document.getElementById("confessNo");
    const success = document.getElementById("confessionSuccess");
    const scene   = document.querySelector(".confessionScene");

    yesBtn.addEventListener("click", () => {
        success.classList.add("visible");
        yesBtn.style.display = "none";
        noBtn.style.display  = "none";
        for(let i = 0; i < 60; i++) _confessionStar(scene);
        setTimeout(nextScene, 5000);
    });

    // NO button evades — desktop hover/mousemove proximity, mobile touch
    const moveNoAway = () => {
        const rect  = scene.getBoundingClientRect();
        const btnW  = noBtn.offsetWidth;
        const btnH  = noBtn.offsetHeight;
        const maxX  = Math.max(rect.width  - btnW  - 24, 24);
        const maxY  = Math.max(rect.height - btnH  - 24, 24);
    ...
    noBtn.addEventListener("pointerdown", (e) => { e.preventDefault(); moveNoAway(); });
...
function _confessionStar(container){
    const s = document.createElement("div");
    s.className = "confessionStar";
...
/*=========================================================
    REVEAL SCENE  —  a few quiet reasons, tap to see
=========================================================*/
const REVEAL_DATA = [
    { icon:"☀️", title:"On the Grey Days", back:"Somehow just hearing from you turns the whole day around. Still don't know how that works." },
    { icon:"😄", title:"The Ridiculous Jokes", back:"The dumbest, most pointless jokes just land differently with you. No idea why." },
    { icon:"🤍", title:"That Quiet Feeling", back:"There's a certain kind of calm that only shows up around you. Hard to explain, easy to notice." }
];

function showReveal(){
    loadTemplate("reveal-template");
    _runReveal();
}

async function _runReveal(){
    changeSky("night");
    const scene = document.querySelector(".revealScene");

    for(const d of REVEAL_DATA){
        const card = document.createElement("div");
        card.className = "flipCard revealCard";
        card.innerHTML = `
          <div class="flipCardInner">
            <div class="flipFront"><span class="cardIcon">${d.icon}</span><h2>${d.title}</h2></div>
            <div class="flipBack"><p>${d.back}</p></div>
    ...
    const cards = scene.querySelectorAll(".flipCard");
    await wait(300);
    for(const c of cards){
        c.style.transition = "opacity 0.5s, transform 0.5s";
        c.style.opacity = "1";
    ...
    nextBtn.className = "acceptBtn";
    nextBtn.textContent = "Continue →";
    nextBtn.style.cssText = "opacity:0;transition:opacity 0.5s;margin-top:12px;";
